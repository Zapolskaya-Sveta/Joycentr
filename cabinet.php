...
if ($user['role'] === 'psychologist') {
    if (isset($_POST['update_psych_schedule'])) {
        ...
        echo "<script>localStorage.setItem('flashToast', JSON.stringify({msg: 'График работы успешно обновлен.', isError: false})); window.location.href='cabinet.php';</script>"; exit;
    }
    ...
    ->execute([$firstName, $patronymic, $lastName, $spec, $exp, $edu, $desc, $imagePath, $directions, $b1t, $b1d, $b2t, $b2d, $specId]);
        echo "<script>localStorage.setItem('flashToast', JSON.stringify({msg: 'Профиль успешно сохранен.', isError: false})); window.location.href='cabinet.php';</script>"; exit;
    }
}
...
if (isset($_POST['add_slot']) || isset($_POST['admin_add_slot'])) {
    ...
    echo "<script>localStorage.setItem('flashToast', JSON.stringify({msg: 'Окно для записи успешно открыто!', isError: false})); window.location.href='cabinet.php';</script>"; exit;
}

if (isset($_POST['notify_waitlist'])) {
    $groupId = (int)$_POST['group_id'];
    $grp = $pdo->query("SELECT title, event_date FROM therapy_groups WHERE id = $groupId")->fetch();
    $waitlist = $pdo->query("SELECT u.email, u.phone, u.name FROM group_participants p JOIN users u ON p.user_id = u.id WHERE p.group_id = $groupId AND p.status = 'waitlist'")->fetchAll();
    if(count($waitlist) > 0) {
        ...
        try {
            ...
            $mail->Username = 'zapolskaaveta@gmail.com'; $mail->Password = 'asjpytxeyhnpuhcr'; 
            ...
            $mail->isHTML(true); $mail->Subject = 'Уведомление листа ожидания | J.O.Y.';
            foreach($waitlist as $w) {
                if (!empty($w['email'])) {
                    $mail->clearAddresses(); $mail->addAddress($w['email']);
                    $msg = "Здравствуйте, {$w['name']}! К сожалению, на групповую терапию <strong>'{$grp['title']}'</strong> так и не освободилось мест.";
                    $mail->Body = "<div style='font-family: Arial, sans-serif; color: #3D3935; border-left: 4px solid #E0C6AD; padding-left: 15px;'><p>$msg</p></div>";
                    $mail->send();
                }
            }
        } catch (Exception $e) {}
    }
    echo "<script>localStorage.setItem('flashToast', JSON.stringify({msg: 'Извинения успешно отправлены списку ожидания.', isError: false})); window.location.href='cabinet.php';</script>"; exit;
}
...
if (isset($_GET['cancel_group_app'])) {
    ...
    if ($groupId > 0 && $user['role'] === 'client') {
        ...
        if ($part) {
            ...
            echo "<script>localStorage.setItem('flashToast', JSON.stringify({msg: 'Вы успешно отменили запись.', isError: false})); window.location.href='cabinet.php';</script>"; exit;
        }
    }
}
...
if (isset($_POST['update_profile'])) {
    ...
    if ($checkEmail->fetch()) { echo "<script>localStorage.setItem('flashToast', JSON.stringify({msg: 'Ошибка: Этот Email уже используется!', isError: true})); window.location.href='cabinet.php';</script>"; exit; }
    $dob = !empty($_POST['dob']) ? $_POST['dob'] : null;
    if ($user['role'] === 'admin') {
        $pdo->prepare("UPDATE users SET name=?, email=? WHERE id=?")->execute([$_POST['name'], $email, $user['id']]);
    } else {
        $pdo->prepare("UPDATE users SET surname=?, name=?, patronymic=?, dob=?, phone=?, email=? WHERE id=?")
            ->execute([$_POST['surname'], $_POST['name'], $_POST['patronymic'], $dob, $_POST['phone'], $email, $user['id']]);
    }
    echo "<script>localStorage.setItem('flashToast', JSON.stringify({msg: 'Данные профиля успешно обновлены.', isError: false})); window.location.href='cabinet.php';</script>"; exit;
}

if (isset($_POST['change_password'])) {
    $oldPass = $_POST['old_password']; $newPass = $_POST['new_password'];
    if (strlen($newPass) < 6) { echo "<script>localStorage.setItem('flashToast', JSON.stringify({msg: 'Ошибка: Новый пароль должен быть не менее 6 символов!', isError: true})); window.location.href='cabinet.php';</script>"; exit; }
    if (password_verify($oldPass, $user['password'])) {
        $hash = password_hash($newPass, PASSWORD_DEFAULT);
        $pdo->prepare("UPDATE users SET password=? WHERE id=?")->execute([$hash, $user['id']]);
        echo "<script>localStorage.setItem('flashToast', JSON.stringify({msg: 'Пароль успешно изменен.', isError: false})); window.location.href='cabinet.php';</script>"; exit;
    } else { echo "<script>localStorage.setItem('flashToast', JSON.stringify({msg: 'Текущий пароль введен неверно!', isError: true})); window.location.href='cabinet.php';</script>"; exit; }
}
...
if (isset($_GET['cancel_app']) && $user['role'] === 'client') {
    ...
    $pdo->prepare("UPDATE schedule SET is_booked = 0, appointment_id = NULL WHERE appointment_id = ?")->execute([$appId]);
    $pdo->prepare("UPDATE appointments SET status='canceled', appointment_time = NULL WHERE id = ? AND user_id = ?")->execute([$appId, $user['id']]);
    echo "<script>localStorage.setItem('flashToast', JSON.stringify({msg: 'Запись отменена.', isError: false})); window.location.href='cabinet.php';</script>"; exit;
}
...
if (isset($_POST['change_app_status']) && ($user['role'] === 'admin' || $user['role'] === 'psychologist')) {
    ...
    if (isset($_POST['new_time']) && !empty($_POST['new_time'])) { $pdo->prepare("UPDATE appointments SET appointment_time=? WHERE id=?")->execute([$_POST['new_time'], $appId]); }
    ...
    if ($newStatus === 'canceled' || $newStatus === 'completed') {
        ...
        } else { $pdo->prepare("DELETE FROM schedule WHERE appointment_id = ?")->execute([$appId]); }
    }
    echo "<script>localStorage.setItem('flashToast', JSON.stringify({msg: 'Данные визита обновлены.', isError: false})); window.location.href='cabinet.php';</script>"; exit;
}
...
if (isset($_POST['save_group'])) {
    ...
    echo "<script>localStorage.setItem('flashToast', JSON.stringify({msg: 'Группа успешно сохранена.', isError: false})); window.location.href='cabinet.php';</script>"; exit;
}
...
if (isset($_GET['delete_group'])) {
    ...
    echo "<script>localStorage.setItem('flashToast', JSON.stringify({msg: 'Группа удалена.', isError: false})); window.location.href='cabinet.php';</script>"; exit;
}
...
if (isset($_GET['remove_participant'])) {
    ...
    echo "<script>localStorage.setItem('flashToast', JSON.stringify({msg: 'Участник исключен.', isError: false})); window.location.href='cabinet.php';</script>"; exit;
}
...
if ($user['role'] === 'psychologist') {
    if (isset($_GET['delete_slot'])) {
        ...
        echo "<script>localStorage.setItem('flashToast', JSON.stringify({msg: 'Слот удален.', isError: false})); window.location.href='cabinet.php';</script>"; exit;
    }
}

if ($user['role'] === 'admin' || $user['role'] === 'psychologist') {
    if (isset($_POST['assign_time'])) { $pdo->prepare("UPDATE appointments SET appointment_time=?, status='confirmed' WHERE id=?")->execute([$_POST['time'], $_POST['app_id']]); header("Location: cabinet.php"); exit; }
}
...
if ($user['role'] === 'admin') {
    if (isset($_GET['approve_order'])) { $ordId = (int)$_GET['approve_order']; $pdo->prepare("UPDATE orders SET status='completed' WHERE id=?")->execute([$ordId]);
        echo "<script>localStorage.setItem('flashToast', JSON.stringify({msg: 'Доступ открыт!', isError: false})); window.location.href='cabinet.php';</script>"; exit;
    }
    if (isset($_GET['delete_admin_app'])) {
        $appId = (int)$_GET['delete_admin_app']; $pdo->prepare("UPDATE schedule SET is_booked=0, appointment_id=NULL WHERE appointment_id=?")->execute([$appId]);
        $pdo->prepare("UPDATE appointments SET is_deleted=1 WHERE id=?")->execute([$appId]);
        echo "<script>localStorage.setItem('flashToast', JSON.stringify({msg: 'Заявка скрыта.', isError: false})); window.location.href='cabinet.php';</script>"; exit;
    }
    if (isset($_GET['delete_user'])) { $delId = (int)$_GET['delete_user']; if ($delId !== (int)$user['id']) $pdo->prepare("DELETE FROM users WHERE id = ?")->execute([$delId]); echo "<script>localStorage.setItem('flashToast', JSON.stringify({msg: 'Пользователь удален.', isError: false})); window.location.href='cabinet.php';</script>"; exit; }
    if (isset($_GET['delete_order'])) { $ordId = (int)$_GET['delete_order']; $pdo->prepare("DELETE FROM order_items WHERE order_id=?")->execute([$ordId]); $pdo->prepare("DELETE FROM orders WHERE id=?")->execute([$ordId]);
        echo "<script>localStorage.setItem('flashToast', JSON.stringify({msg: 'Заказ удален.', isError: false})); window.location.href='cabinet.php';</script>"; exit;
    }
    ...
    if (isset($_GET['approve_review'])) { $pdo->prepare("UPDATE reviews SET status = 'approved' WHERE id = ?")->execute([$_GET['approve_review']]); header("Location: cabinet.php"); exit; }
    if (isset($_GET['reject_review'])) { $pdo->prepare("DELETE FROM reviews WHERE id = ?")->execute([$_GET['reject_review']]); header("Location: cabinet.php"); exit; }
    if (isset($_GET['delete_post'])) { $pdo->prepare("DELETE FROM posts WHERE id=?")->execute([$_GET['delete_post']]); header("Location: cabinet.php"); exit; }
    if (isset($_GET['approve_post'])) { $pdo->prepare("UPDATE posts SET status = 'published' WHERE id=?")->execute([$_GET['approve_post']]); header("Location: cabinet.php"); exit; }
    if (isset($_GET['delete_product'])) { $pdo->prepare("DELETE FROM products WHERE id=?")->execute([$_GET['delete_product']]); header("Location: cabinet.php"); exit; }
    ...
    if (isset($_POST['save_system_settings'])) {
        ...
        echo "<script>localStorage.setItem('flashToast', JSON.stringify({msg: 'Настройки сайта сохранены.', isError: false})); window.location.href='cabinet.php';</script>"; exit;
    }
    if (isset($_POST['save_faq'])) {
        ...
        echo "<script>localStorage.setItem('flashToast', JSON.stringify({msg: 'Вопрос сохранен.', isError: false})); window.location.href='cabinet.php';</script>"; exit;
    }
    if (isset($_GET['delete_faq'])) { $pdo->prepare("DELETE FROM faq WHERE id=?")->execute([$_GET['delete_faq']]); header("Location: cabinet.php"); exit; }
    if (isset($_POST['add_room'])) { $pdo->prepare("INSERT INTO rooms (name) VALUES (?)")->execute([trim($_POST['room_name'])]); header("Location: cabinet.php"); exit; }
    if (isset($_GET['delete_room'])) { $pdo->prepare("DELETE FROM rooms WHERE id=?")->execute([$_GET['delete_room']]); header("Location: cabinet.php"); exit; }
    if (isset($_POST['add_topic'])) { $pdo->prepare("INSERT INTO topics (title) VALUES (?)")->execute([trim($_POST['topic_title'])]); header("Location: cabinet.php"); exit; }
    if (isset($_GET['delete_topic'])) { $pdo->prepare("DELETE FROM topics WHERE id=?")->execute([$_GET['delete_topic']]); header("Location: cabinet.php"); exit; }
}
...
<div class="cabinet-wrapper">
    <aside class="joy-sidebar">
        ...
        <?php else: ?>
            ...
            <?php if($clientOrdersCount > 0): ?><a href="#" class="joy-nav-item" onclick="showTab('my-orders', this)"><div class="joy-nav-icon"><i class="fas fa-history"></i></div><div class="joy-nav-text">Мои материалы</div></a><?php endif; ?>
    ...
    <main class="joy-content">
                    ...
                    <div class="card-panel h-100">
                        ...
                        <form method="POST">
                            ...
                            <?php if($user['role'] !== 'admin'): ?>
                                ...
                                <div class="mb-3"><label class="font-weight-bold text-muted small">Дата рождения</label><input type="date" name="dob" value="<?= $user['dob'] ?>" class="form-control joy-input"></div>
                                <div class="mb-3"><label class="font-weight-bold text-muted small">Телефон</label><input type="tel" name="phone" value="<?= htmlspecialchars($user['phone'] ?? '') ?>" class="form-control joy-input"></div>
                            <?php endif; ?>
                            ...
                            <td class="align-middle px-4 text-right table-actions-cell">
                                <?php if($user['role'] === 'admin'): ?>
                                    <?php if($p['status'] !== 'published'): ?>
                                        <a href="?approve_post=<?= $p['id'] ?>" class="action-btn btn-approve" title="Опубликовать" onclick="event.preventDefault(); joyConfirm('Опубликовать эту статью?', () => { window.location.href = this.href; })"><i class="fas fa-check"></i></a>
                                    <?php endif; ?>
                                    <button class="action-btn btn-edit" onclick='editPost(<?= json_encode($p) ?>)' title="Редактировать"><i class="fas fa-pen"></i></button>
                                    <a href="?delete_post=<?= $p['id'] ?>" class="action-btn btn-delete" title="Удалить" onclick="event.preventDefault(); joyConfirm('Удалить статью?', () => { window.location.href = this.href; })"><i class="fas fa-trash-alt"></i></a>
                                <?php else: ?>
                                    <button class="action-btn btn-edit" onclick='editPost(<?= json_encode($p) ?>)' title="Редактировать"><i class="fas fa-pen"></i></button>
                        ...
                        <div class="row align-items-center">
                            <div class="col-md-6">
                               <h5 class="font-weight-bold mb-1">
                                    <?= htmlspecialchars(preg_replace('/^\[.*?\]\s*/', '', $app['topic'])) ?>
                                    <?php if($filterStatus == 'callback'): ?><span class="badge badge-callback-small">Перезвонить</span><?php endif; ?>
                                </h5>
                                <?php if($filterStatus != 'callback'): ?>
                                <span class="badge mb-3 d-inline-block badge-service-tag"><i class="fas fa-briefcase-medical mr-1"></i> <?= htmlspecialchars($app['service_type'] ?? 'Очная сессия') ?></span>
                                <?php endif; ?>
                            ...
                            <div class="col-md-6 text-md-right mt-3 mt-md-0">
                                    ...
                                    <?php if(!empty($app['appointment_time'])): ?>
                                        <div class="mt-3 card-action-request-bg">
                                            ...
                                            <div class="d-flex justify-content-end gap-2">
                                                <a href="?reject_app=<?= $app['id'] ?>" class="btn btn-outline-danger btn-sm btn-round-action-reject" onclick="event.preventDefault(); joyConfirm('Отклонить заявку?', () => { window.location.href = this.href; })">Отклонить</a>
                                ...
                                <div class="mt-3"><a href="?delete_admin_app=<?= $app['id'] ?>" class="text-danger small text-underline" onclick="event.preventDefault(); joyConfirm('Удалить заявку?', () => { window.location.href = this.href; })"><i class="fas fa-trash"></i> Удалить</a></div>
                            </div>
                        </div>
                    </div>
                <?php endforeach; ?>
            ...
            <div class="row">
                <div class="col-lg-8">
                    <div class="card-panel mb-5">
                        <div class="d-flex justify-content-between align-items-center border-bottom pb-3 mb-4"><h5 class="m-0 font-weight-bold">Сводка графиков работы</h5></div>
                        <div class="table-responsive">
                            <table class="table table-hover m-0 joy-table">
                                <thead class="table-joy-head"><tr><th class="border-0 text-muted font-weight-normal">Психолог</th><th class="border-0 text-muted font-weight-normal">Заявленный график</th></tr></thead>
                                <tbody>
                                <?php foreach($adminSpecs as $as): $schd = $pdo->query("SELECT work_schedule FROM specialists WHERE id=" . $as['id'])->fetchColumn(); ?>
                                <tr class="table-joy-row">
                    ...
                    <div class="card-panel">
                        <div class="d-flex justify-content-between align-items-center border-bottom pb-3 mb-4"><h5 class="m-0 font-weight-bold">Свободные окна</h5></div>
                        <div class="mb-4 d-flex align-items-center flex-wrap joy-filter-bar">
                            <select class="form-control form-control-sm joy-select filter-select-spec" id="filterAdminSchedSpec" onchange="applyAdminSchedFilters()">
                                <option value="all">Все психологи</option><?php foreach($adminSpecs as $as) echo "<option value='{$as['id']}'>{$as['first_name']} {$as['last_name']}</option>"; ?>
                            </select>
                        ...
                        <div class="row" id="adminSchedContainer">
                            <?php $allSlots = $pdo->query("SELECT sch.*, s.first_name, s.last_name FROM schedule sch JOIN specialists s ON sch.specialist_id = s.id WHERE sch.slot_datetime > NOW() ORDER BY sch.slot_datetime ASC")->fetchAll();
                            if(count($allSlots) == 0) echo "<div class='col-12'><div class='empty-state'><i class='far fa-clock empty-icon'></i><h4 class='empty-text'>Расписание пусто</h4></div></div>";
                            foreach($allSlots as $slot): 
                                $dateStr = date('Y-m-d', strtotime($slot['slot_datetime'])); $niceDate = date('d.m.Y', strtotime($slot['slot_datetime'])); $timeStr = date('H:i', strtotime($slot['slot_datetime']));
                                $bookedOpacity = $slot['is_booked'] ? 'item-opacity-low' : '';
                            ?>
                            <div class="col-md-4 col-sm-6 mb-3 admin-sched-item" data-spec="<?= $slot['specialist_id'] ?>" data-date="<?= $dateStr ?>">
                                <div class="p-3 border rounded text-center h-100 d-flex flex-column slot-card-admin-bg <?= $bookedOpacity ?>">
                                    ...
                                    <h4 class="mb-1 text-dark-joy"><?= $timeStr ?></h4><p class="text-muted small mb-2"><?= $niceDate ?></p>
                                    <div class="mt-auto">
                                        <?php if($slot['is_booked']): ?><span class="badge badge-booked-full">Занято</span>
                                        <?php else: ?><span class="badge badge-free-slot">Свободно</span><?php endif; ?>
                                        <a href="?delete_admin_slot=<?= $slot['id'] ?>" class="text-danger small text-underline" onclick="event.preventDefault(); joyConfirm('Удалить слот?', () => { window.location.href = this.href; })">Удалить</a>
                                    </div>
                                </div>
                            </div>
                            <?php endforeach; ?>
                ...
                <div class="col-lg-4">
                    <div class="card-panel sticky-top-admin-panel">
                        <h5 class="mb-4 font-weight-bold text-dark-joy"><i class="fas fa-plus-circle text-muted mr-2"></i>Новое окно</h5>
                        <form method="POST">
                            <div class="mb-3"><label class="text-muted small mb-1 font-weight-bold">Психолог</label>
                                <select name="admin_add_slot_spec" class="form-control joy-select" required><option value="" disabled selected>Выберите...</option><?php foreach($adminSpecs as $as) echo "<option value='{$as['id']}'>{$as['first_name']} {$as['last_name']}</option>"; ?></select>
                            </div>
                            <div class="mb-4"><label class="text-muted small mb-1 font-weight-bold">Дата и время</label><input type="datetime-local" name="slot_time" class="form-control joy-input" required></div>
            ...
            <div>
                ...
                <div class="card-panel card-accent-border mb-4">
                    ...
                    <div class="mt-3 d-flex justify-content-end gap-2">
                        <a href="?reject_review=<?= $rev['id'] ?>" class="btn btn-outline-danger btn-sm btn-round-action-reject-spam" onclick="event.preventDefault(); joyConfirm('Удалить отзыв?', () => { window.location.href = this.href; })">В спам</a>
                        <a href="?approve_review=<?= $rev['id'] ?>" class="main-button small-btn">Одобрить на сайт</a>
                    </div>
                </div>
                <?php endforeach; ?>
            </div>
                ...
                <div class="card-panel mb-4 admin-order-item <?= $rowOpacityClass ?>" data-time="<?= strtotime($order['created_at']) ?>" data-date="<?= date('Y-m-d', strtotime($order['created_at'])) ?>">
                    <div class="d-flex justify-content-between align-items-center border-bottom pb-3 mb-3">
                        ...
                        <div class="d-flex align-items-center">
                            ...
                            <a href="?delete_order=<?= $order['id'] ?>" class="action-btn btn-delete" onclick="event.preventDefault(); joyConfirm('Удалить заказ?', () => { window.location.href = this.href; })" title="Удалить заказ"><i class="fas fa-trash-alt"></i></a>
                        </div>
                    </div>
                    ...
                    <div class="d-flex justify-content-between align-items-center mt-3 border-top pt-3">
                        <?php if(!$isCompleted): ?>
                            ...
                            <a href="?approve_order=<?= $order['id'] ?>" class="main-button small-btn btn-no-decor" onclick="event.preventDefault(); joyConfirm('Открыть доступ?', () => { window.location.href = this.href; })">Открыть доступ</a>
                        <?php else: ?>
                ...
                <div class="table-responsive">
                    <table class="table table-hover m-0 joy-table">
                        ...
                        <tr class="crm-user-item table-joy-row" data-role="<?= $u['role'] ?>" data-search="<?= mb_strtolower($u['name'].' '.$u['surname'].' '.$u['email'].' '.$phoneStr) ?>">
                            ...
                            <td class="align-middle px-4 text-right table-actions-cell">
                                <a href="?reset_password=<?= $u['id'] ?>" class="action-btn btn-edit" onclick="event.preventDefault(); joyConfirm('Сбросить пароль?', () => { window.location.href = this.href; })" title="Сбросить пароль"><i class="fas fa-key"></i></a>
                                <?php if($u['id'] != $user['id']): ?>
                                    <a href="?delete_user=<?= $u['id'] ?>" class="action-btn btn-delete" onclick="event.preventDefault(); joyConfirm('Удалить пользователя?', () => { window.location.href = this.href; })" title="Удалить"><i class="fas fa-trash-alt"></i></a>
                                <?php endif; ?>
                            </td>
                        </tr>
                        <?php endforeach; ?>
                            ...
                            <tbody>
                                ...
                                <tr class="table-joy-row">
                                    ...
                                    <td class="align-middle px-4 text-right table-actions-cell">
                                        ...
                                        <a href="?delete_faq=<?= $f['id'] ?>" class="action-btn btn-delete" onclick="event.preventDefault(); joyConfirm('Удалить вопрос?', () => { window.location.href = this.href; })" title="Удалить"><i class="fas fa-trash-alt"></i></a>
                                    </td>
                                </tr>
                        ...
                        <div class="card-panel h-100">
                            ...
                            <ul class="list-group">
                                ...
                                <li class="list-group-item d-flex justify-content-between align-items-center">
                                    ...
                                    <a href="?delete_room=<?= $r['id'] ?>" class="text-danger" onclick="event.preventDefault(); joyConfirm('Удалить?', () => { window.location.href = this.href; })"><i class="fas fa-times-circle"></i></a>
                                </li>
                                <?php endforeach; } catch (Exception $e) {} ?>
                            </ul>
                        </div>
                        ...
                        <div class="card-panel h-100">
                            ...
                            <ul class="list-group">
                                ...
                                <li class="list-group-item d-flex justify-content-between align-items-center">
                                    ...
                                    <a href="?delete_topic=<?= $t['id'] ?>" class="text-danger" onclick="event.preventDefault(); joyConfirm('Удалить?', () => { window.location.href = this.href; })"><i class="fas fa-times-circle"></i></a>
                                </li>
                                <?php endforeach; } catch (Exception $e) {} ?>
                            </ul>
                        </div>
        ...
        if ($specialistData) {
            $rawApps = $pdo->query("SELECT a.topic, u.name, a.appointment_time, a.guest_name, a.status FROM appointments a LEFT JOIN users u ON a.user_id = u.id WHERE a.specialist_id = {$specialistData['id']} AND a.status IN ('assigned', 'confirmed', 'canceled') AND a.is_deleted = 0")->fetchAll();
            foreach($rawApps as $ra) if($ra['appointment_time']) $myAppsData[] = ['time' => $ra['appointment_time'], 'client' => $ra['name'] ?: $ra['guest_name'], 'topic' => $ra['topic'], 'date' => date('Y-m-d', strtotime($ra['appointment_time'])), 'isGroup' => false, 'status' => $ra['status']];
            $rawGrps = $pdo->query("SELECT title, event_date FROM therapy_groups WHERE spec_id = {$specialistData['id']}")->fetchAll();
            foreach($rawGrps as $rg) if($rg['event_date']) $myAppsData[] = ['time' => $rg['event_date'], 'client' => 'Группа', 'topic' => $rg['title'], 'date' => date('Y-m-d', strtotime($rg['event_date'])), 'isGroup' => true, 'status' => 'confirmed'];
            $rawSlots = $pdo->query("SELECT slot_datetime, notes FROM schedule WHERE specialist_id = {$specialistData['id']} AND is_booked = 0 AND slot_datetime > NOW()")->fetchAll();
            foreach($rawSlots as $rs) $mySlotsData[] = ['time' => $rs['slot_datetime'], 'notes' => $rs['notes']];
        }
        ?>
        <script>const globalPsychApps = <?= json_encode($myAppsData) ?>; const globalPsychSlots = <?= json_encode($mySlotsData) ?>;</script>
                ...
                <div id="psychAppsContainer">
                    <?php if ($specialistData): 
                        ...
                        foreach($appointmentsList as $app): 
                            ...
                            $rowOpacityClass = ($app['status'] == 'completed' || $app['status'] == 'canceled') ? 'item-opacity-low' : 'card-accent-border';
                    ?>
                    <div class="card-panel psych-app-item mb-4 <?= $rowOpacityClass ?>" data-status="<?= $app['status'] ?>" data-service="<?= htmlspecialchars($app['service_type'] ?? '') ?>" data-client="<?= mb_strtolower($clientName) ?>" data-time="<?= strtotime($app['created_at']) ?>" data-appdate="<?= date('Y-m-d', strtotime(!empty($app['appointment_time']) ? $app['appointment_time'] : $app['created_at'])) ?>">
                        <div class="row align-items-center">
                            <div class="col-md-6">
                                <h4 class="font-weight-bold mb-1"><?= htmlspecialchars(preg_replace('/^\[.*?\]\s*/', '', $app['topic'])) ?></h4>
                                <span class="badge mb-3 d-inline-block badge-service-tag"><i class="fas fa-briefcase-medical mr-1"></i> <?= htmlspecialchars($app['service_type'] ?? 'Очная сессия') ?></span>
                            ...
                            <div class="col-md-6 text-md-right mt-3 mt-md-0">
                                ...
                                <?php else: ?>
                                    <div class="p-3 rounded text-left d-inline-block card-action-request-bg box-min-width-300">
                                        <h5 class="mb-2 text-dark-joy"><?= !empty($app['appointment_time']) ? date('d.m.Y в H:i', strtotime($app['appointment_time'])) : 'Ожидание' ?></h5>
                                        <form method="POST"><input type="hidden" name="app_id" value="<?= $app['id'] ?>">
                                            <div class="d-flex gap-2 mb-2">
            ...
            <div id="psych-sub-1" class="inner-content">
                ...
                <div class="calendar-wrapper mb-5">
                    ...
                    <div class="calendar-right card-panel p-4 m-0 flex-grow-1 cal-day-view-bg" id="psychDayView">
                        <h5 class="mb-4 font-weight-bold text-center border-bottom pb-3" id="dayViewTitle">Расписание на день</h5>
                        <div class="daily-timeline daily-timeline-scroll pr-2" id="dailyTimeline"></div>
                        <form method="POST" id="quickSlotForm" style="display:none;"><input type="hidden" name="slot_time" value=""><input type="hidden" name="spec_id" value="<?= $specialistData['id'] ?>"><input type="hidden" name="add_slot" value="1"></form>
                    </div>
                </div>
            ...
            <div class="row">
                ...
                <div class="card-panel mb-4 <?= $rowOpacityClass ?>">
                    ...
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <?php else: ?><span class="badge badge-success-booking"><i class="fas fa-check-circle mr-1"></i> Успешная запись</span><a href="?cancel_group_app=<?= $g['group_id'] ?>" class="btn btn-outline-danger btn-sm btn-round-action-reject" onclick="event.preventDefault(); joyConfirm('Отменить запись?', () => { window.location.href = this.href; })">Отменить запись</a>
                        <?php endif; ?>
                    </div>
                </div>
                <?php endwhile; ?>
                ...
                <div class="card-panel mb-4 <?= $rowOpacityClass ?>">
                    <div class="d-flex justify-content-between align-items-start border-bottom pb-3 mb-3">
                        <div>
                            <h4 class="m-0 font-weight-bold mb-1">
                                <?= htmlspecialchars(preg_replace('/^\[.*?\]\s*/', '', $a['topic'])) ?>
                                <?php if(strpos($a['topic'], 'Обратная связь') !== false): ?><span class="badge badge-callback-status">Перезвонить</span><?php endif; ?>
                            </h4>
                            <?php if(strpos($a['topic'], 'Обратная связь') === false): ?>
                            <span class="badge mb-2 d-inline-block badge-service-tag"><i class="fas fa-briefcase-medical mr-1"></i> <?= htmlspecialchars($a['service_type'] ?? 'Очная сессия') ?></span>
                            <?php endif; ?>
                    ...
                    <div class="d-flex justify-content-between align-items-center">
                        <?php if($a['status'] == 'assigned' || $a['status'] == 'confirmed'): ?>
                            ...
                            <a href="?cancel_app=<?= $a['id'] ?>" class="btn btn-outline-danger btn-sm btn-round-action-reject" onclick="event.preventDefault(); joyConfirm('Отменить запись?', () => { window.location.href = this.href; })">Отменить запись</a>
...
<div id="tab-cart" class="content-tab" style="display: none;">
        ...
        <div id="tab-my-orders" class="content-tab" style="display:none;">
            ...
            <div class="row">
                ...
                while($ord = $myOrders->fetch()):
                    $isPending = ($ord['status'] == 'pending'); $rowOpacityClass = $isPending ? 'item-opacity-low' : 'card-accent-border';
                ?>
                <div class="card-panel mb-4 <?= $rowOpacityClass ?>">
                    <div class="d-flex justify-content-between align-items-center border-bottom pb-3 mb-3">
                        <h4 class="m-0 font-weight-bold">Заказ #<?= $ord['id'] ?> <span class="text-muted ml-2 font-size-09rem font-weight-normal"><?= date('d.m.Y H:i', strtotime($ord['created_at'])) ?></span></h4>
                        <div class="text-right">
                            ...
                            <span class="badge badge-price-display-alt"><?= $ord['total_price'] ?> BYN</span>
                        </div>
                    </div>
                    <ul class="list-unstyled mb-4 text-muted font-size-1rem">
                        ...
                        $items = $pdo->prepare($sqlItems); $items->execute([$ord['id']]); 
                        while($i = $items->fetch()): 
                        ?>
                            <li class='mb-2 d-flex justify-content-between align-items-center p-3 rounded order-item-access-bg'>
                                <span class="font-weight-bold text-dark-joy"><i class="fas fa-play-circle mr-2 icon-accent-joy"></i> <?= htmlspecialchars($i['product_title']) ?></span>
                                <?php if($isPending): ?><span class="text-muted small"><i class="fas fa-lock mr-1"></i> Доступ закрыт куратором</span>
                                <?php elseif(!empty($i['access_link'])): ?><a href="<?= htmlspecialchars($i['access_link']) ?>" target="_blank" class="main-button small-btn btn-no-decor">Открыть материал</a>
                                <?php else: ?><span class="badge badge-light border text-muted">Ссылка формируется</span><?php endif; ?>
                            </li>
                        <?php endwhile; ?>
                    </ul>
                    <div class="text-right"><a href="receipt.php?id=<?= $ord['id'] ?>" target="_blank" class="btn btn-outline-dark btn-sm btn-round-action-receipt"><i class="fas fa-file-invoice mr-1"></i> Скачать чек PDF</a></div>
                </div>
                <?php endwhile; ?>
                </div>
            </div>
...
<!-- МОДАЛКИ (Оставляем структуру для JS) -->
...
<div class="clients-modal-overlay" id="clientsModalOverlay">
    <div class="clients-modal-box">
        ...
        <form id="notifForm" action="send_notification.php" method="POST" class="notif-form-flex-layout h-100 d-flex flex-column">
            <div class="d-flex mb-3 flex-wrap gap-10 flex-shrink-0">
                <div class="filter-input-search-wrapper flex-grow-1">
                    <i class="fas fa-search search-icon-input"></i>
                ...
                <select id="notifGroupFilter" class="form-control joy-select flex-grow-1" onchange="filterNotifClients()">
                    <option value="all">Все записи</option><option value="tomorrow">Завтрашние</option><option value="session">Индивидуальные</option>
                    <?php try { $allGrs = $pdo->query("SELECT id, title FROM therapy_groups")->fetchAll(); foreach($allGrs as $g) echo "<option value='group_{$g['id']}'>Группа: ".htmlspecialchars($g['title'])."</option>"; } catch (Exception $e) {} ?>
                </select>
            </div>
            ...
            <div class="clients-modal-body modal-body-scrollable">
                ...
                try {
                    ...
                    $actApps = $pdo->query("SELECT a.id, u.name, u.surname, a.guest_name, a.appointment_time, a.guest_phone FROM appointments a LEFT JOIN users u ON a.user_id=u.id WHERE a.status = 'confirmed' AND a.appointment_time > NOW() ORDER BY a.appointment_time ASC")->fetchAll();
                    foreach($actApps as $act): 
                        $n = $act['name'] ? $act['name'].' '.$act['surname'] : $act['guest_name']; $isTomorrow = (!empty($act['appointment_time']) && date('Y-m-d', strtotime($act['appointment_time'])) === $tomorrowStr) ? 'yes' : 'no';
                ?>
                    <div class="custom-control custom-checkbox notif-client-item mb-2 p-2 rounded client-item-bg-white" data-name="<?= mb_strtolower($n) ?>" data-group="session" data-tomorrow="<?= $isTomorrow ?>">
                        ...
                        <label class="custom-control-label cursor-pointer w-100" for="client_app_<?= $act['id'] ?>"><strong class="text-dark-joy"><?= $n ?></strong> <span class="text-muted ml-2">(<?= $act['guest_phone'] ?>) - <?= date('d.m H:i', strtotime($act['appointment_time'])) ?></span></label>
                    </div>
                <?php endforeach; 
                    $actGrp = $pdo->query("SELECT p.id as pid, p.group_id, u.name, u.surname, u.phone, g.title, g.event_date FROM group_participants p JOIN users u ON p.user_id = u.id JOIN therapy_groups g ON p.group_id = g.id WHERE p.status = 'active' AND g.event_date > NOW()")->fetchAll();
                    foreach($actGrp as $ag): 
                        $n = $ag['name'].' '.$ag['surname']; $isTomorrow = (date('Y-m-d', strtotime($ag['event_date'])) === $tomorrowStr) ? 'yes' : 'no';
                ?>
                    <div class="custom-control custom-checkbox notif-client-item mb-2 p-2 rounded client-item-bg-accent" data-name="<?= mb_strtolower($n) ?>" data-group="group_<?= $ag['group_id'] ?>" data-tomorrow="<?= $isTomorrow ?>">
                        ...
                        <label class="custom-control-label cursor-pointer w-100" for="client_grp_<?= $ag['pid'] ?>"><strong class="text-dark-joy"><?= $n ?></strong> <span class="text-accent-joy ml-2">(<?= $ag['phone'] ?>) - Группа <?= date('d.m H:i', strtotime($ag['event_date'])) ?></span></label>
                    </div>
                <?php endforeach; } catch (Exception $e) {} ?>
            </div>
            <div class="mt-1 flex-shrink-0"><label class="font-weight-bold small text-muted mb-1">Текст уведомления (Шаблон)</label><textarea name="message" class="form-control joy-input no-resize" rows="3" required>Здравствуйте, {имя}! Напоминаем о вашей записи в J.O.Y. Center на {дата} в {время}. {услуга}. Ждем вас по адресу: {адрес}.</textarea></div>
...
<div class="custom-confirm-overlay" id="orderSuccessModal">
    <div class="custom-confirm-box modal-box-max-500">
        ...
        <h3 class="font-tenor text-dark-joy mb-3">Оплата прошла успешно!</h3>
        <p class="font-lato font-size-105rem text-muted line-height-16 mb-4">Ваш заказ принят в обработку. Доступ будет открыт в течение <b>24 часов</b>.<br><br>Следите за статусом во вкладке <b>«Мои материалы»</b>.</p>
